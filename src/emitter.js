...
Lecturer.prototype.emit = function (eventName, data) {
    var lastDotIdx = eventName.lastIndexOf('.');
    if (lastDotIdx >= 0) {
        this.emit(eventName.substring(0, lastDotIdx), data);
    }
    this._getListenersForEvent(eventName).forEach(function (listener) {
        listener.callListener(data);
    });
};
...
Listener.prototype.callListener = function (data) {
    this._callCount++;
    if ((this._maxCalls && this._maxCalls >= this._callCount) ||
        ...
        this._callback.call(this._student, data);
    }
};
